...
class MCPErrorHandler {
  ...
  Future<bool> _recoverTimeout(MCPError error, int attempt) async {
    try {
      ...
      print('⏱️ Timeout recovery: waiting ${backoffDelay.inSeconds}s before retry $attempt');
      ...
      print('❌ Timeout recovery failed: $e');
      return false;
    }
  }
  ...
  Future<bool> _recoverAuthFailure(MCPError error, int attempt) async {
    try {
      ...
      if (oauthProvider != null && _oauthService != null) {
        print('🔐 Attempting OAuth token refresh for $oauthProvider (attempt $attempt)');
  ...
  Future<bool> _recoverRateLimit(MCPError error, int attempt) async {
    try {
      ...
      print('🚫 Rate limit hit, waiting ${waitTime.inMinutes}m ${waitTime.inSeconds % 60}s');
      await Future.delayed(waitTime);
      
      return true; // Rate limits typically resolve with time
    } catch (e) {
      print('❌ Rate limit recovery failed: $e');
      return false;
    }
  }
  ...
  Future<bool> _recoverProcessStart(MCPError error, int attempt) async {
    try {
      ...
      print('🔧 Attempting process start recovery for $serverId (attempt $attempt)');
      
      // Get fresh configuration
      final catalogEntry = _catalogService!.getCatalogEntry(serverId);
      if (catalogEntry == null) {
        print('❌ Catalog entry not found for $serverId');
        return false;
      }
      ...
      print('❌ Process start recovery failed: $e');
      return false;
    }
  }
...
final mcpErrorHandlerProvider = Provider<MCPErrorHandler>((ref) {
  ...
  try {
    ...
    print('⚠️ Some error recovery services unavailable: $e');
  }
